...
public class view_book extends JFrame {
	...
	private String bookfile = "C:\\Users\\THANH\\Desktop\\GK_OOP\\test\\book_data.txt";

	private void saveToFile(int STT, String id, String tieu_de, String tac_gia, String nam_xuat_ban) {

		try {
			PrintWriter writer = new PrintWriter(new FileWriter(bookfile, true));
			writer.println(STT + "," + id + "," + tieu_de + "," + tac_gia + "," + nam_xuat_ban);
			writer.flush();
			writer.close();
	...
	public view_book() {
		...
		bt_add.setBounds(521, 11, 131, 52);
		panel.add(bt_add);
		...
		bt_add.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (name.isEmpty() == false && tacj.isEmpty() == false && namxbb.isEmpty() == false) {
					Object[] rowData = { STt, id, name, tacj, namxbb };
					((DefaultTableModel) table.getModel()).addRow(rowData);
					saveToFile(STT, id, name, tacj, namxbb);
					tieu_de.setText("");
					tac_gia.setText("");
		...
		bt_remove.setBounds(521, 68, 131, 52);
		panel.add(bt_remove);
		...
		cb_Button.setBounds(521, 126, 131, 56);
		panel.add(cb_Button);
		Come_back(cb_Button);
		...
		table.setModel(new DefaultTableModel(
				dataArray,
				new String[] {
						"STT", "ID", "Tiêu Đề", "Tác giả", "Năm xuất bản"
				}));
		JScrollPane scrollPane = new JScrollPane(table);
		scrollPane.setBounds(0, 186, 708, 476);
